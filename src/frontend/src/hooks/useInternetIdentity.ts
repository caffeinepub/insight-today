...
export function InternetIdentityProvider({
  ...
  const handleLoginError = useCallback(
    (maybeError?: string) => {
      setErrorMessage(
        maybeError ?? "Login failed. Please allow popups and try again.",
      );
    },
    [setErrorMessage],
  );
  ...
  const login = useCallback(() => {
    if (!authClient) {
      setErrorMessage(
        "Auth is still starting up. Please wait a moment and try again.",
      );
      return;
    }
    ...
    // Already authenticated with a valid session -- just set state, no popup needed
    if (
  ...
  const clear = useCallback(() => {
    ...
    void authClient
      ...
      .then(() => {
        ...
        setStatus("idle");
        setError(undefined);
        // re-create auth client fresh after logout
        void createAuthClient(createOptions).then(setAuthClient);
      })
  ...
  }, [authClient, createOptions, setErrorMessage]);
